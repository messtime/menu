      ...
      <el-collapse @change="handleChange" v-model="activeNames">
        <el-collapse-item name="1">
          ...
          >
            ...
            >
              <div slot="content">
                {{ item.person + mimacode[item.passward] }}
              </div>
              <el-checkbox-button
                :label="mimacode[item.passward] + item.bank + item.number[1]"
                :key="item.number[2]"
  ...
  methods: {
    ...
    handleCheckAllChange(val) {
      ...
      for (let i = 0; i < val.length; i++) {
        newStr[i] = val[i].replace("198", "!");
        newStr[i] = newStr[i].replace("35", "@");
        newStr[i] = newStr[i].replace("62", "^");
      }
      this.$message({
